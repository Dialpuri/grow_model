...
class Params:
    ...
    pdb_dir: str = "/vault/pdb"
    pdb_dir_extracted: bool = False
...
def get_pdb_structure(
    ...
    if Params.pdb_dir_extracted:
        ...
        pdb_path = os.path.join(Params.pdb_dir, pdb_code[1:3], f"pdb{pdb_code}.ent.gz")
...
def analyse_pdb(
    ...
    for chain in structure[0]:
        for n_res in range(len(chain) - 1):
            ...
            superpose_ = gemmi.superpose_positions([gemmi.Position(*probe_data["base"][x]) for x in Params.alignment_atoms], 
                                                   [residue.sole_atom(x).pos for x in Params.alignment_atoms])
            
            
            average_middle = np.mean(list(probe_data["base"].values()), axis=0)

            transform = gemmi.Transform(superpose_.transform.mat, superpose_.transform.vec - gemmi.Position(*average_middle) + gemmi.Position(8,8,8))
            
            
            array = np.zeros((16,16,16), dtype=np.float32)
            grid.interpolate_values(array, transform.inverse())
            
            yield array, *torsions
            # translate to a fix position
            # Get a 3D grid around that 
            ...
            # output_structure.write_pdb("debug/probe_points_2.pdb")
...
def generate_dataset(train_or_test: str):
    ...
    probe_file = "data/probe_C3C4O3_3.json"
